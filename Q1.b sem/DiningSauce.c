...
void *dine (void *args){ //The created threads will execute this function simultaneously
	int *pnum = (int*) args;
	  //Passing the philosopher number
	while(1)
	{ a++;
    if(a>1000)
    {
	...
	if(!(funcbowl[0]&&(&bowl[0])))
	{
  funcbowl[0]=1;
	sem_wait(&bowl[0]);
	printf("Philospher number %d has picked up bowl number %s \n",*pnum+1,"1");
	}
  else if(!(funcbowl[1]&&(&bowl[1])))
	{
  funcbowl[1]=1;
	sem_wait(&bowl[1]);
	printf("Philospher number %d has picked up bowl number %s \n",*pnum+1,"2");
		...
		sleep(1); //Indicate Eating for 2 seconds
	...
	if((funcbowl[1]&&(&bowl[1])))
	{
  funcbowl[1]=0;
	sem_post(&bowl[1]);
	printf("Philospher number %d has kept the bowl number %s \n",*pnum+1,"2");
...
int main()
{
    ...
    func[3]=0;
    func[4]=0;
    funcbowl[0]=0;  //Function of bowl is 0 that means available
    funcbowl[1]=0;
   ...
   for(i=0;i<2;i++)
   {
    sem_init(&bowl[i],0,1);   //Initializing bowl semaphore as 1
   }
